...
export function SiteHeader({ current, transparent = false }: HeaderProps) {
  ...
  return <>
    <header className={`topbar${transparent ? " transparent" : ""}`}>
      ...
      <div className="topbar-actions"><button className="search-button" type="button" onClick={() => setSearchOpen(true)} aria-label="Search anime"><span className="search-glyph" aria-hidden="true">⌕</span><span>Search anime</span><kbd>⌘ K</kbd></button><Link className="profile-button" href="/profile" aria-label="Open profile">L</Link></div>
    </header>
